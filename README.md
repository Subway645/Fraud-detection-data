...
> 注意：数据挖掘词从全量 utterance 提取，因此 `matcher.py` 用全量词表评测的召回率（85.8%/80.8%）存在特征构建泄漏，**不是真实泛化能力**。真实泛化见下方验收结果的 G 列（62.2%/64.7%）。
...
> 注：matcher.py 生产口径（全量词表）fraud 85.8% / ad 80.8% / 误报 0，含特征泄漏，仅日常检查用，非汇报口径。
